...
class SLL(object):
    ...
    def sec_to_last(self):
        """
        method returns the value of the second to last node in the list.
        """
        if not self.__head:
            result = "Empty Linked List"
        else:
    ...
    def filter(self, low, high):
        """
        method removes values between low and high from the list
        """
        if not self.__head:
            result = "Empty Linked List"
            return result
            ...
            while (cur):
                if ((cur.data < low) or (cur.data > high)):
                    if prev:
                        prev.next = cur.next
                        cur.next = None
                        cur = prev.next
    ...
    def remNode(self, data):
        """
        method removes the node containing data. Different from the 'remove' method in that it
        - cannot remove last node
        - doesn't keep track of prev node during traversal
        - doesn't need head node or prev node for removal
        This is equivalent to a standalone function which receives a data node and removes it from the list without needing the list Head pointer
        """
        # Finding the relevant node. Uses Head
        cur = self.__head
        while ((cur.data != data) and (cur.next != None)):
            cur = cur.next

        # The previous while loop will give us the node to be removed.
        # Use that as the temperary "head" node to move all further "values" 1 spot to the left.
        if ((cur.data == data) and (cur.next)):
            nxt = cur.next
            while (nxt.next):
                cur.data = nxt.data
                cur = nxt
